...
namespace Agency.FrameworkControls
{
    ...
    public class QueryBuilder : WebControl
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                var objectType = Type.GetType(QueryObjectName);
                ...
                foreach (var attribute in QueryFields)
                {
                    var qfa = attribute as QueryFieldAttribute;
                    if (qfa != null)
                    {
        ...
        void _ddlComparison_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (_ddlComparison.SelectedIndex > 0)
            {
                ...
                foreach (var attribute in QueryFields)
                {
                    ...
                    {
                        var qfa = (QueryFieldAttribute)attribute;

                        if (qfa.FieldName == valueFieldName)
                        {
                            lookupFieldName = qfa.LookupFieldName ?? valueFieldName;
                        }
                    }
                }
                ...
                var objectType = Type.GetType(QueryObjectName);
                object listObject = Activator.CreateInstance(objectType);
                var data = (List<LookupData>)objectType.InvokeMember("GetLookup", BindingFlags.InvokeMethod, null, listObject, new object[] { lookupFieldName, valueFieldName });
        ...
        void _ddlFields_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (_ddlFields.SelectedIndex > 0)
            {
                ...
                var queryFieldType = QueryFieldAttribute.QueryFieldTypeEnum.NotSet;
        ...
        void _btnRemoveLine_Click(object sender, EventArgs e)
        {
            if (_lstWhereClause.SelectedIndex > -1)
            {
                ...
                switch (_lstWhereClause.Items.Count)
                {
                    case 0:
        ...
        private void CreateDropDowns()
        {
            ...
            var tcField = new TableCell();
            tcField.Text = "Select a field";
            trHeader.Cells.Add(tcField);

            var tcComparison = new TableCell();
